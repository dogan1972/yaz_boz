...
class _EllerSayfasiState extends State<EllerSayfasi> {
  ...
  void _oyuncuAlanlariniHazirla(int? oyunId, {List<El>? mevcutEller}) {
    ...
    for (var c in _karControllers) {
      c.dispose();
    }
    for (var c in _zararControllers) {
      c.dispose();
    }
    for (var c in _gostergeControllers) {
      c.dispose();
    }

    setState(() {
      _aktifOyuncular = seciliOyun.oyuncu
          .split(', ')
          .where((o) => o.isNotEmpty)
          .toList();

      _karControllers = List.generate(
        _aktifOyuncular.length,
        (index) => TextEditingController(),
      );
      _zararControllers = List.generate(
        _aktifOyuncular.length,
        (index) => TextEditingController(),
      );
      _gostergeControllers = List.generate(
        _aktifOyuncular.length,
        (index) => TextEditingController(),
      );
        ...
        for (int i = 0; i < _aktifOyuncular.length; i++) {
          ...
          if (elKayit.elTarih.isNotEmpty) {
            ...
            if (elKayit.gosterge != null) {
              _gostergeControllers[i].text = (elKayit.gosterge! * -1)
                  .toString();
            } else {
  ...
  void _elFormuGoster({
    ...
    List<El>? elGrubuElemanlari,
  }) {
    final duzenlemeModu =
        guncellenecekTarih != null && elGrubuElemanlari != null;
      ...
      builder: (dialogContext) => StatefulBuilder(
        builder: (context, setDialogState) {
          ...
          Future<void> kaydetmeIslemi() async {
            ...
            final seciliOyun = _oyunlar.firstWhere(
            ...
            final List<Map<String, dynamic>> satirSayisiRes = await db.rawQuery(
              'SELECT COUNT(*) as toplam FROM eller WHERE oyunId = ?',
              [_seciliOyunId],
            );
            ...
            if (!duzenlemeModu && mevcutElSayisi >= hedefElSayisi) {
              ...
              messenger.showSnackBar(
                SnackBar(
                  content: Text(
                    "⚠️ Maksimum el sayısına ($hedefElSayisi El) ulaştı!",
                  ),
            ...
            if (duzenlemeModu) {
              ...
              for (int i = 0; i < _aktifOyuncular.length; i++) {
                ...
                final pZarar =
                    int.tryParse(_zararControllers[i].text.trim()) ?? 0;
                yeniSkorlar[_aktifOyuncular[i]] = (pKar * -1) + pZarar;

                final hamGosterge =
                    int.tryParse(_gostergeControllers[i].text.trim()) ?? 0;
                await db.update(
                  'eller',
              ...
              for (int i = 0; i < _aktifOyuncular.length; i++) {
                ...
                El yeniEl = El(
                  ...
                  oyuncu3: _aktifOyuncular.length > 2
                      ? _aktifOyuncular[(i + 2) % _aktifOyuncular.length]
                      : null,
                  oyuncu4: _aktifOyuncular.length > 3
                      ? _aktifOyuncular[(i + 3) % _aktifOyuncular.length]
                      : null,
                  elSkor: netSkor,
                  gosterge: hamGosterge * -1,
                );
            ...
            final sonElKontrol =
                (mevcutElSayisi + 1) >= hedefElSayisi && !duzenlemeModu;
            final oyunSonucu = await Oyun.oyunSonucunuHesapla(
              oyunId: _seciliOyunId!,
              isHighestWins: _isHighestWins,
              kaliciKapat: sonElKontrol,
            );
            ...
            return Column(
              ...
              children: [
                isYatay
                    ...
                    : Column(
                        mainAxisSize: MainAxisSize.min,
                        children: dikeyUstElemanlar,
                      ),
                isYatay
                    ? Table(
                        children: List.generate(
                          (_aktifOyuncular.length / 2).ceil(),
                          (rowIdx) {
                            int solIdx = rowIdx * 2;
                            int sagIdx = solIdx + 1;
                            return TableRow(
                    ...
                    : Column(
                        ...
                        children: List.generate(
                          _aktifOyuncular.length,
                          (index) => oyuncuGirisKutusuOlustur(index),
                        ),
                      ),
              ],
            );
                    ...
                    children: [
                      Expanded(
                        ...
                        child: SingleChildScrollView(
                          child: formGovdesiOlustur(),
                        ),
                      ),
                      const VerticalDivider(width: 12, thickness: 1),
                      Expanded(
                        flex: 1,
                        child: yataySagPanelOlustur(kaydetmeIslemi),
                      ),
                    ],
                  ),
                ),
          ...
          return AlertDialog(
            ...
            title: Text(
              duzenlemeModu ? 'El Skorlarını Düzenle' : 'Yeni El Skoru Girişi',
            ),
            content: SizedBox(
      ...
      body: FutureBuilder<List<El>>(
        ...
        builder: (context, snapshot) {
          ...
          for (var list in oyuncuGostergeleri.values) {
            if (list.length > maxGostergeSatiri) {
              maxGostergeSatiri = list.length;
            }
          }

          // 🚀 GÖRSELDEKİ TAŞMA VE ASİMETRİ HATASINI ÇÖZEN EN KRİTİK ALAN:
          // İlk sütun (El No) sabit 65 piksel kaplar.
          // Diğer tüm oyuncu sütunları isim uzunluğuna bakılmaksızın kalan alanı EŞİT (Flex) olarak paylaşır.
                    ...
                    child: Container(
                      decoration: BoxDecoration(
                        border: Border.all(
                          color: Colors.black,
                          width: 2,
                        ), // Çizdiğiniz şablona uygun kalın siyah kenarlık
                      ),
                      child: Table(
                        ...
                        children: [
                          // A. SÜTUN BAŞLIKLARI (OYUNCU İSİMLERİ)
                          TableRow(
                            decoration: BoxDecoration(
                                ...
                                (oyuncu) => Padding(
                                  ...
                                  child: Text(
                                    ...
                                    overflow: TextOverflow
                                        .ellipsis, // İsim sığmazsa taşma hatası vermez, '...' koyar
                                    style: const TextStyle(
                                      fontWeight: FontWeight.bold,
                                      fontSize: 13,
                                    ),
                          ...
                          // C. EL SKORLARI SATIRLARI (Tarih verilerine göre kronolojik)
                                ...
                                ..._aktifOyuncular.map((oyuncuAdi) {
                                  ...
                                  return GestureDetector(
                                    ...
                                    child: Container(
                                      padding: const EdgeInsets.all(8.0),
                                      color: Colors
                                          .transparent, // Hücrenin her yerine tıklanabilmesini sağlar
                                      child: Text(
                          ...
                          // D. MAVİ RENKLİ ALT TOPLAM SATIRI (EN ALT KAPANIŞ)
                          TableRow(
                            decoration: BoxDecoration(
        ...
        onPressed: () {
          if (_oyunlar.isEmpty) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text(
                  "Skor girebilmek için aktif bir oyun kurmalısınız!",
                ),
              ),
            );
            return;
